	...
	if ( is_single() ) {
		...
		foreach ( $match[0] as $item ) {
			...
			if ( strpos( $item[0], $cat[0] ) !== false ) {
				...
				$replace .= '<li data-pid="h3' . $h2_number . '"' . ( isset( $parent['h1'] ) ? ' data-parent="' . $parent['h1'] . '"' : '' ) . '>
				...
				$replace .= '<li data-pid="h4' . $h3_number . '"' . ( isset( $parent['h2'] ) ? ' data-parent="' . $parent['h2'] . '"' : '' ) . '>
 ...
 * 获取修改时间
 */
function get_new_post_modified_time( $format ) {

	/**
	 * 判断修改时间是否在发布时间之后
	 */
	if ( strtotime( get_post_modified_time( "Y-m-d H:i:s" ) ) < strtotime( get_post_time( "Y-m-d H:i:s" ) ) ) {
